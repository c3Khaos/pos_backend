...
def hw_sale_ids_subquery():
    ...
    return (
        ...
        .filter(Product.category.in_(HARDWARE_CATEGORIES))
        .distinct()
        .scalar_subquery()
    )
    ...
    def get(self):
        ...
        advances_row = (
            db.session.query(
                ...
                func.coalesce(
                    func.sum(CashAdvance.amount - func.coalesce(CashAdvance.amount_returned, 0)),
                    0,
                ).label('owed'),
            )
            .filter(
...
class HardwareLowStockResource(Resource):
    ...
    def get(self):
        ...
        return {
            "out_of_stock": [
                {"id": p.id, "name": p.name, "stock": float(p.stock), "category": p.category}
